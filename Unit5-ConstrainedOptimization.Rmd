...
P = makeFun( 100*x^(4/11) * y^(7/11) ~ x&y)
...
```{r fig.show='hide'}
plotFun(P(x,y)~x&y, xlim=c(20,40), ylim=c(60,100), filled=FALSE, levels=seq(4700,6600,300))
```
...
After some experimentation, I realized that $P(x,y)=5300$ is the contour that is tangent $ Q(x,y)=500$. So the optimal point is $(26,80)$ and the optimal output is 53.
...
plotFun(P(x,y)~x&y, xlim=c(0,100), ylim=c(0,100), filled=FALSE, levels=seq(1500,8500,1000))
```
...
The optimal point is at $(21,40)$ and the optimal production is approximately $f(21,40)=3500$. (This is a slight under-estimate, but it's pretty close.)
